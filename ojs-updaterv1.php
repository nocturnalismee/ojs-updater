 ...
 *  [FIX #01] mysqldump password via --defaults-extra-file (tidak expose di ps)
 *  [FIX #02] Rollback sekarang hapus file baru + restore file lama
 *  [FIX #03] canExecCli() cek exec DAN shell_exec
 *  [FIX #04] Lockout file pakai flock() (race condition safe)
 *  [FIX #05] extractOjsZip() panggil _protectDir()
 *  [FIX #06] CSRF token di-regenerate setelah login
 *  [FIX #07] Login sekarang verify CSRF token
 *  [FIX #08] Maintenance mode regex handle quoted values
 *  [FIX #09] copyDir() preserve permissions
 *  [FIX #10] copyDir() handle symlinks (tidak follow)
 *  [FIX #11] $out diinisialisasi [] sebelum setiap exec()
 *  [FIX #12] mergePluginsDir() handle files bukan hanya dir
 *  [FIX #13] PDO backup pakai fetch() batch (tidak OOM)
 *  [FIX #14] PDO backup export views
 *  [FIX #15] findOjsSubdir() verifikasi index.php
 *  [FIX #16] classifyUpgrade() handle version unknown
 *  [FIX #17] runOjsDbUpgrade() pakai proc_open (tanpa chdir)
 *  [FIX #18] Log rotation 5MB
 *  [FIX #19] Cache tryFetchLiveOjsVersions() 1 jam
 *  [FIX #20] backupImportantFiles() log .htaccess failure
 *  [FIX #21] PDO pakai MYSQL_ATTR_CONNECT_TIMEOUT
 *  [FIX #22] set_time_limit() cek function_exists
 *  [FIX #23] Disk space check sebelum backup
 *  [FIX #24] Warning jika open_basedir aktif
 */
...
define('LOG_MAX_SIZE',      5 * 1024 * 1024); // 5MB rotation
...
define('VERSION_CACHE_FILE',TMP_WORK_DIR . '/ojs_versions_cache.json');
...
/**
 * [FIX #23] Cek disk space tersedia
 */
function checkDiskSpace(int $requiredBytes): bool
{
    $free = @disk_free_space(BACKUP_BASE_DIR);
    return $free !== false && $free > $requiredBytes;
}

/**
 * [FIX #24] Deteksi open_basedir restriction
 */
function isOpenBasedirRestricted(): bool
{
    $openBasedir = ini_get('open_basedir');
    if (!$openBasedir) return false;
    // Cek apakah BACKUP_BASE_DIR dalam allowed paths
    $paths = explode(PATH_SEPARATOR, $openBasedir);
    $backupReal = realpath(BACKUP_BASE_DIR) ?: BACKUP_BASE_DIR;
    foreach ($paths as $p) {
        $p = trim($p);
        if ($p === '' ) continue;
        $pReal = realpath($p) ?: $p;
        if (str_starts_with($backupReal, $pReal)) return false;
    }
    return true;
}
...
function ojsLog(string $level, string $msg): void
{
    ...
    // [FIX #18] Log rotation: rotate jika melebihi LOG_MAX_SIZE
...
/**
 * [FIX #04] Semua fungsi lockout sekarang menggunakan flock()
 *           untuk mencegah race condition pada concurrent requests.
 */
function getLockoutData(): array
{
    $default = ['attempts' => 0, 'locked_until' => 0, 'last_attempt' => 0];
    if (!file_exists(LOCKOUT_FILE)) return $default;
...
{
    // Read-modify-write dengan exclusive lock
    if (!is_dir(dirname(LOCKOUT_FILE))) @mkdir(dirname(LOCKOUT_FILE), 0750, true);
    $fp = @fopen(LOCKOUT_FILE, 'c');
    if (!$fp) return;
    flock($fp, LOCK_EX);
    ...
    $content = stream_get_contents($fp);
    $d = @json_decode($content, true);
    if (!is_array($d)) $d = ['attempts' => 0, 'locked_until' => 0, 'last_attempt' => 0];
    ...
    if ($d['attempts'] >= MAX_LOGIN_ATTEMPTS) {
        ...
        // Log sambil masih pegang lock (tidak deadlock karena ojsLog pakai file berbeda)
    }
...
/**
 * [FIX #06] Regenerate CSRF token (dipanggil setelah login berhasil)
 */
...
/**
 * [FIX #09] copyDir sekarang preserve permissions
 * [FIX #10] copyDir sekarang handle symlinks (copy sebagai symlink, tidak follow)
 */
function copyDir(string $src, string $dst): bool
{
    ...
    foreach ($iter as $item) {
        ...
        // [FIX #10] Handle symlinks — copy sebagai symlink, jangan follow
        if ($item->isLink()) {
            $linkTarget = @readlink($item->getPathname());
            if ($linkTarget !== false && !file_exists($target)) {
        ...
        if ($item->isDir()) {
            if (!is_dir($target)) {
                ...
                // [FIX #09] Preserve directory permissions
                @chmod($target, $item->getPerms() & 0777);
            }
        } elseif (!copy($item->getPathname(), $target)) {
            throw new RuntimeException("Gagal copy: " . $item->getPathname());
        } else {
            // [FIX #09] Preserve file permissions
            @chmod($target, $item->getPerms() & 0666);
        }
    }
    return true;
}
...
function removeDir(string $dir): bool
{
    ...
    try {
        ...
        foreach ($iter as $f) {
            if ($f->isLink()) { @unlink($f->getPathname()); continue; }
...
/**
 * [FIX #03] canExecCli sekarang cek exec DAN shell_exec
 */
function canExecCli(): bool
{
    ...
    foreach (['exec', 'shell_exec'] as $fn) {
        ...
        if (in_array($fn, $disabled, true)) return false;
    }
    return true;
}
...
/**
 * [FIX #17] runOjsDbUpgrade sekarang pakai proc_open (tidak chdir)
 */
...
/**
 * [FIX #08] Regex sekarang handle quoted values: installed = "On" / installed = "Off"
 */
function getMaintenanceStatus(string $ojsRoot): bool
{
    $f = $ojsRoot . '/config.inc.php';
    if (!file_exists($f)) return false;
    return (bool)preg_match('/^\s*installed\s*=\s*["\']?Off["\']?\s*$/mi', (string)file_get_contents($f));
}
...
function setMaintenanceMode(string $ojsRoot, bool $enable): bool
{
    ...
    if ($enable) {
        // [FIX #08] Handle quoted "On" → Off
        $new = preg_replace('/^(\s*installed\s*=\s*)["\']?On["\']?\s*$/mi', '${1}Off', $content);
    } else {
        // [FIX #08] Handle quoted "Off" → On
        $new = preg_replace('/^(\s*installed\s*=\s*)["\']?Off["\']?\s*$/mi', '${1}On', $content);
    }
...
function backupDatabase(array $cfg, string $outDir): array
{
    ...
    // [FIX #23] Cek disk space (estimasi 500MB minimum)
    if (!checkDiskSpace(500 * 1024 * 1024)) {
        $result['error'] = 'Disk space tidak mencukupi untuk backup DB (min 500MB free)';
        return $result;
    }
    ...
    if ($mysqldump) {
        ...
        // [FIX #01] Gunakan --defaults-extra-file agar password tidak terekspos di process list
        $defaultsFile = $outDir . '/.my.cnf.' . getmypid() . '.tmp';
        $cnfContent = "[client]\nhost={$dbHost}\nuser={$dbUser}\n";
        if ($dbPass !== '') {
            // Escape double quotes in password
            $escapedPass = str_replace('"', '\\"', $dbPass);
            $cnfContent .= "password=\"{$escapedPass}\"\n";
        }
        file_put_contents($defaultsFile, $cnfContent);
        chmod($defaultsFile, 0600);

        $cmd = sprintf(
            '%s --defaults-extra-file=%s --single-transaction --routines --triggers --add-drop-table --no-tablespaces %s > %s 2>&1',
        ...
        $out = []; // [FIX #11]
        exec($cmd, $out, $code);
        @unlink($defaultsFile); // Hapus file credentials segera

        if ($code === 0 && file_exists($outFile) && filesize($outFile) > 100) {
            $gzip = findBin('gzip');
            if ($gzip) {
                $gzOut = []; // [FIX #11]
                exec(escapeshellarg($gzip) . ' -f ' . escapeshellarg($outFile), $gzOut, $gzCode);
        ...
        @unlink($outFile);
        @unlink($defaultsFile); // Pastikan terhapus jika gagal
    }
    ...
    try {
        ...
        // [FIX #21] Gunakan MYSQL_ATTR_CONNECT_TIMEOUT jika tersedia
        $options = [
        ...
        foreach ($tables as $table) {
            ...
            // [FIX #13] Gunakan fetch() satu-per-satu, tidak load semua ke memory
            ...
            if ($hasData) fwrite($handle, "\n");
        }

        // [FIX #14] Export views
        $viewsResult = $pdo->query("SHOW FULL TABLES WHERE Table_type = 'VIEW'");
        if ($viewsResult) {
            $views = $viewsResult->fetchAll(PDO::FETCH_COLUMN, 0);
            foreach ($views as $view) {
                try {
                    $createView = $pdo->query("SHOW CREATE VIEW `{$view}`")->fetch(PDO::FETCH_NUM);
                    if ($createView) {
                        fwrite($handle, "-- View: {$view}\n");
                        fwrite($handle, "DROP VIEW IF EXISTS `{$view}`;\n");
                        fwrite($handle, $createView[1] . ";\n\n");
                    }
                } catch (Exception) {
                    // Skip view jika tidak bisa export (misal: permission)
                }
            }
        }
...
function attemptDownload(string $url, string $destFile): ?string
{
    ...
    if (ini_get('allow_url_fopen')) {
        if (function_exists('set_time_limit')) @set_time_limit(0); // [FIX #22]
        $ctx  = stream_context_create(['http' => ['timeout' => 600, 'user_agent' => 'OJS-Updater/2.1']]);
...
/**
 * [FIX #15] findOjsSubdir sekarang verifikasi index.php sebelum return
 */
function findOjsSubdir(string $extractDir): string
{
    // Cek apakah extractDir itu sendiri adalah root OJS
    ...
    // Prioritas 1: direktori dengan 'ojs' di nama DAN ada index.php
    foreach ($subdirs as $d) {
        if (stripos(basename($d), 'ojs') !== false && file_exists($d . '/index.php')) {
            return $d;
        }
    }

    // Prioritas 2: direktori manapun yang punya index.php
    foreach ($subdirs as $d) {
    ...
    // Fallback: direktori dengan 'ojs' di nama (tanpa verifikasi)
    foreach ($subdirs as $d) {
...
/**
 * [FIX #05] extractOjsZip sekarang panggil _protectDir()
 */
function extractOjsZip(string $zipPath, string $extractTo): array
{
    ...
    if (!is_dir($extractTo)) mkdir($extractTo, 0750, true);
    _protectDir($extractTo); // [FIX #05]
    $zip = new ZipArchive();
...
function backupImportantFiles(string $ojsRoot, string $backupDir): array
{
    ...
    // [FIX #23] Cek disk space sebelum backup
    if (!checkDiskSpace(200 * 1024 * 1024)) {
        $result['error'] = 'Disk space tidak mencukupi untuk backup file (min 200MB free)';
        return $result;
    }
    ...
    if (file_exists($ojsRoot . '/config.inc.php')) {
        if (copy($ojsRoot . '/config.inc.php', $backupDir . '/config.inc.php')) {
    ...
    // [FIX #20] .htaccess — log warning jika gagal, bukan diam-diam skip
    if (file_exists($ojsRoot . '/.htaccess')) {
        if (copy($ojsRoot . '/.htaccess', $backupDir . '/.htaccess')) {
            $result['backed_up'][] = '.htaccess';
        } else {
            $result['warnings'][] = 'Gagal backup .htaccess';
        }
    }
...
/**
 * [FIX #12] mergePluginsDir sekarang handle files (bukan hanya directories)
 *           di level kategori dan level plugin
 */
function mergePluginsDir(string $newPluginsDir, string $existingPluginsDir): array
{
    ...
    try {
        foreach (new DirectoryIterator($newPluginsDir) as $cat) {
            if ($cat->isDot()) continue;
            $catName     = $cat->getFilename();
            $newCatPath  = $cat->getPathname();
            $existCatDir = $existingPluginsDir . '/' . $catName;
            $corePl      = $coreList[$catName] ?? [];

            // [FIX #12] Handle files di level kategori (mis. index.php)
            if ($cat->isFile()) {
                // Selalu update file di level kategori
                if (!file_exists($existCatDir) || filemtime($newCatPath) > filemtime($existCatDir)) {
                    @copy($newCatPath, $existCatDir);
                    $result['updated'][] = $catName;
                }
                continue;
            }
            ...
            if (!is_dir($existCatDir)) {
                copyDir($newCatPath, $existCatDir);
                $result['updated'][] = "$catName/ (kategori baru)";
                continue;
            }

            foreach (new DirectoryIterator($newCatPath) as $plugin) {
                if ($plugin->isDot()) continue;
                $pName     = $plugin->getFilename();
                $newPath   = $plugin->getPathname();
                $existPath = $existCatDir . '/' . $pName;

                // [FIX #12] Handle files di level plugin (mis. version.xml, index.php)
                if ($plugin->isFile()) {
                    // Update file pendukung plugin
                    if (!file_exists($existPath) || in_array($pName, ['version.xml', 'index.php'], true)) {
                        @copy($newPath, $existPath);
...
/**
 * [FIX #02] Rollback sekarang:
 *   - Menghapus file/dir yang baru ditambahkan (tidak ada sebelumnya)
 *   - Me-restore file/dir yang sudah ada sebelumnya dari backup
 */
function applyOjsUpdate(string $newOjsDir, string $ojsRoot, bool $dryRun = false): array
{
    ...
    $appliedItems = []; // [FIX #02] Track SEMUA item, termasuk yang baru
    ...
    try {
        foreach (new DirectoryIterator($newOjsDir) as $item) {
            ...
            // [FIX #02] Backup untuk rollback — track baik yang sudah ada maupun yang baru
            $existedBefore = file_exists($dstPath) || is_dir($dstPath);
            if ($existedBefore) {
                $rb = $rollbackDir . '/' . $name;
                is_dir($dstPath) ? copyDir($dstPath, $rb) : copy($dstPath, $rb);
            }
            // [FIX #02] Simpan info untuk rollback (termasuk item baru yang belum ada)
            $appliedItems[] = [
                'name'    => $name,
                'type'    => $item->isDir() ? 'dir' : 'file',
                'rollback' => $existedBefore ? ($rollbackDir . '/' . $name) : null,
        ...
        // [FIX #02] Rollback yang lebih robust:
        //   - Hapus item yang baru ditambahkan (tidak ada sebelum update)
        //   - Restore item yang sudah ada dari backup
        $rbOk = true;
        foreach (array_reverse($appliedItems) as $app) {
            $dst = $ojsRoot . DIRECTORY_SEPARATOR . $app['name'];

            // Hapus item saat ini (baik baru maupun hasil replace)
            if ($app['type'] === 'dir') { removeDir($dst); } else { @unlink($dst); }

            // [FIX #02] Jika item sudah ada sebelum update, restore dari backup
            if ($app['existed'] && $app['rollback'] !== null) {
                try {
                    if ($app['type'] === 'dir') {
                        copyDir($app['rollback'], $dst);
                    } else {
                        copy($app['rollback'], $dst);
                    }
                } catch (Exception $re) {
                    $rbOk = false;
            ...
            // [FIX #02] Jika item baru (tidak ada sebelum update), biarkan dihapus (sudah dihapus di atas)
        }
...
/**
 * [FIX #19] Cache hasil fetch ke file dengan TTL 1 jam
 */
function tryFetchLiveOjsVersions(): array
{
    $cacheFile = VERSION_CACHE_FILE;
    $cacheTtl = 3600; // 1 jam
    ...
    $ctx   = stream_context_create(['http' => ['timeout' => 8, 'user_agent' => 'OJS-Updater/2.1', 'ignore_errors' => true]]);
    $found = [];
    ...
    // Simpan cache
    if (is_dir(dirname($cacheFile)) || @mkdir(dirname($cacheFile), 0750, true)) {
...
/**
 * [FIX #16] Handle version "Tidak diketahui" — return 'major' sebagai default aman
 */
function classifyUpgrade(string $cur, string $tgt): string
{
    $norm = static function (string $v): array {
        ...
        // [FIX #16] Return null untuk version yang tidak bisa di-parse
        return [null, null];
    };
    [$cBase, $cB] = $norm($cur);
    [$tBase, $tB] = $norm($tgt);

    // [FIX #16] Jika current version tidak dikenali, anggap major upgrade
    if ($cBase === null) return 'major';
    if ($tBase === null) return 'reinstall';
...
if (function_exists('set_time_limit')) { // [FIX #22]
...
// ── Login ────────────────────────────────────────────────────
if ($action === 'login') {
    // [FIX #07] Verifikasi CSRF untuk login juga
    if (!verifyCsrfToken()) {
        ...
        if (hash_equals(SCRIPT_PASSWORD, $pwd)) {
            ...
            // [FIX #06] Regenerate CSRF token setelah login
            regenerateCsrfToken();
            clearLockout();
...
// ── Logout ──────────────────────────────────────────────────
if ($action === 'logout') {
    // Logout tidak perlu CSRF check (sudah common practice, tapi bisa ditambah)
    ojsLog('info', 'Logout');
    _destroySession();
...
// [FIX #24] Cek open_basedir
 $openBasedirWarn = checkAuth() ? isOpenBasedirRestricted() : false;
...
<title>OJS Core Updater v2.1</title>
...
<div class="card">
  ...
  <div class="card-body">
    ...
    <?php if ($openBasedirWarn): ?>
    <div class="callout callout-warn" style="margin-bottom:16px;">
      <span class="callout-ico">⚠</span>
      <div><strong>open_basedir aktif:</strong> <code><?= e(ini_get('open_basedir')) ?></code><br>Direktori backup mungkin terbatas. Jika terjadi error path, letakkan backup di dalam path yang diizinkan.</div>
    </div>
    <?php endif; ?>
    ...
    <table class="info-table">
      ...
      <tr>
        <td class="lbl">Disk Free</td>
        <td class="val"><?= checkDiskSpace(0) ? formatBytes((int)@disk_free_space(BACKUP_BASE_DIR)) : '<span style="color:var(--coral)">Tidak terdeteksi</span>' ?></td>
      </tr>
...
<div class="card">
  ...
  <div class="card-body">
    ...
    <div class="notice notice-warn" style="margin-bottom:20px;">
      ...
      Rollback akan: (1) menghapus file baru yang ditambahkan, (2) me-restore file lama dari backup.
    ...
    <div class="sub-step">
      <div>
        ...
        <div class="sub-step-desc">
          ...
          Rollback otomatis (hapus file baru + restore file lama) jika terjadi error.
        </div>
      </div>
      <form method="post">
        <?= csrfField() ?>
...
<div class="card">
  <div class="card-head">
    ...
    <div><h2>Panduan Penggunaan v<?= SCRIPT_VERSION ?></h2><small>alur · keamanan · catatan penting</small></div>
  </div>
  <div class="card-body">
    <div class="guide-steps">
      <div class="guide-step"><div class="guide-step-num">1</div><div class="guide-step-text"><strong>Info & Cek</strong> — Verifikasi PHP, ZIP, backup dir, disk space, dan status keamanan. Aktifkan <strong>Maintenance Mode</strong> agar tidak ada user yang mengakses OJS selama proses.</div></div>
      <div class="guide-step"><div class="guide-step-num">2</div><div class="guide-step-text"><strong>Backup File + Database</strong> — Backup otomatis config.inc.php, public/, .htaccess, files_dir. Plus backup database via mysqldump (password aman via --defaults-extra-file) atau PDO fallback (batch fetch, export views).</div></div>
      <div class="guide-step"><div class="guide-step-num">3</div><div class="guide-step-text"><strong>Siapkan File OJS</strong> — Download otomatis ke server (cache versi 1 jam), upload manual, atau arahkan ke path SSH. Gunakan <strong>Dry Run</strong> untuk preview perubahan sebelum apply.</div></div>
      <div class="guide-step"><div class="guide-step-num">4</div><div class="guide-step-text"><strong>Apply Update</strong> — 4a: Dry Run (preview). 4b: Ganti core (rollback otomatis: hapus file baru + restore file lama). 4c: Clear cache. 4d: DB upgrade hanya jika naik versi.</div></div>
